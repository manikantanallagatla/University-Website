...
<form method="post">
  ...
  $sql_query = "SELECT distinct c.cid, c.name FROM course c, student s, takes t where s.sid = '".$temp_sid."' and s.sid = t.sid and c.cid = t.cid";
  $result = $mysqli->query($sql_query);
...
$coursetounenroll1 = $_POST["coursetounenroll"];
if($coursetounenroll1!=""){
  
  $sql_query = "DELETE from takes where takes.sid = '".$temp_sid."' and takes.cid = '".$coursetounenroll1."'";  //   VALUES('".$coursetoenroll1."' ,'".$temp_sid."')";
  if($mysqli->query($sql_query) === true){
    echo "Succesfully unenrolled!!";
  }else{
    echo "Already uniqid()enrolled!!";
  }
  header("Refresh:0");
}
// $file = "code.txt"; 
  ...
  var treeData = [
  {
    ...
    "children":[
      ...
      if (mysqli_connect_errno()) {
        ...
        $sql_query1 = "SELECT distinct c.name as name FROM student s, course c, takes t where c.cid = t.cid and t.sid = s.sid and s.name = '".$name."'";
        // echo $sql_query1;
